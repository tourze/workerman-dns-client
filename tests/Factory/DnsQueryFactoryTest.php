...
final class DnsQueryFactoryTest extends TestCase
{
    ...
    {
        // 检查工厂方法是否创建了正确的依赖关系
        $fileName = (new \ReflectionClass(DnsQueryFactory::class))->getFileName();
        $this->assertNotFalse($fileName, 'Failed to get DnsQueryFactory file name');
        $source = file_get_contents($fileName);
        $this->assertNotFalse($source, 'Failed to read DnsQueryFactory source file');
